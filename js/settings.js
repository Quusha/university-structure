...
function renderSettings(){
  ...
  c.innerHTML=
    cardHTML('Учётная запись',`<div class="field" style="max-width:420px"><label>Вы вошли как</label><div style="padding:8px 0">${currentUser?('<b>'+esc(currentUser.email)+'</b> · роль <b>'+esc(currentUser.role)+'</b>'):(backendReady()?'<span class="hint">вход не выполнен</span>':'<span class="hint">локальный режим (бэкенд не настроен в config.js)</span>')}</div><small>Роль назначается администратором и проверяется на сервере при каждом действии — переключить её в браузере нельзя.</small>${currentUser?'<div style="margin-top:10px"><button class="btn danger sm" onclick="doLogout()">Выйти</button></div>':''}</div>`)
    +(currentUser&&currentUser.role==='Администратор'?cardHTML('Управление пользователями',`<button class="btn sm" onclick="document.querySelector('#tabs button[data-tab=admin]').click()">📋 Открыть «Пользователи»</button><small style="display:block;margin-top:6px">Выдача ролей, подтверждение и блокировка аккаунтов.</small>`):'')
