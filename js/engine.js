...
function applyRole(){
  ...
  document.querySelectorAll('#tabs button').forEach(b=>{
    const allow=r.tabs==='*'||r.tabs.indexOf(b.dataset.tab)>=0;
...
function logAudit(action,entity,detail){state.audit.unshift({id:uid(),time:new Date().toISOString(),who:state.roleCurrent,action,entity,detail});if(state.audit.length>500)state.audit.length=500;}
